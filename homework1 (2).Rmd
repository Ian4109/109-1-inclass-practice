...
# G5TimeParsed
G5TimeParsed <- paste0(
  Game5Time[[1]], Game5Time[[2]], Game5Time[[3]]
  )
print(G5TimeParsed)
```
...
# TpeJason
TpeJason <- with_tz(
  G5TimeParsed, tzone = "Asia/Taipei"
  )
```
...
ParisLouis <- with_tz(
  G5TimeParsed, tzone = "Europe/Paris"
  )
```
...
as.numeric(chNumbers_r) -> chNumbers_n
sumchNumbers <- sum(chNumbers_n)
print(sumchNumbers)
```
...
```{r ans22}
# sumChNumbersWith0
chNumbersWith0_r <- factor(
  chNumbersWith0, levels =
                      c(
                        "零","壹","貳","參","肆","伍","陸","柒","捌","玖"
                        )
  )
chNumbersWith0 <- as.integer(
  chNumbersWith0_r
  )
...
```{r ans4}
download.file(
  url="https://www.dropbox.com/s/rykdkmz27o1t4dh/IMG_2027.JPG?dl=1",
              destfile="hw1.jpg"
  )
